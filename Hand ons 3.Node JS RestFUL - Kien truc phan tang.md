...
# Mở port (giả sử backend chạy cổng 5000)
EXPOSE 5000
...
## 3. Chỉnh sửa  file `models/account.js` để bổ sung thuộc tính `password`
...
---
### 11. Chỉnh sửa trang frontend (solution) để xác thực

1. Thêm textbox `password` trong trang `solution/index.html`
...
2. Chỉnh sửa `solution/app.js` để thực hiện xác thực
